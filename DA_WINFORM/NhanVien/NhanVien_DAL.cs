...
namespace DA_WINFORM
{
    ...
    class NhanVien_DAL
    {
        ...
        {
            string sql = "SELECT * FROM NHANVIEN WHERE MANV LIKE @keyword OR TENNV LIKE @keyword OR SDT LIKE @keyword";
            SqlConnection con = dataCon.getConnect();
            sqlDA = new SqlDataAdapter(sql, con);
            sqlDA.SelectCommand.Parameters.AddWithValue("@keyword", "%" + keyword + "%");
